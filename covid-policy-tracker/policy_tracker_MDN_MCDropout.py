...
class PolicyTrackerMDN(pl.LightningModule):
    ...
    def MC_Dropout(self, X, sample_size, only_max_component=True):
        ...
        if only_max_component:
            means = torch.zeros(size=(X.shape[0], sample_size))
            variances = torch.zeros(size=(X.shape[0], sample_size))

            for i in range(sample_size):
                if i % 100 == 0:
                    print(f"sample nr {i}/{sample_size}")
            ...
            means = torch.zeros(size=(sample_size, X.shape[0], self.num_gaussians))
            variances = torch.zeros(size=(sample_size, X.shape[0], self.num_gaussians))
            weights = torch.zeros(size=(sample_size, X.shape[0], self.num_gaussians))
